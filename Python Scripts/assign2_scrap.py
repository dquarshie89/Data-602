...
def action(trade):
    ...
    if trade == 'Sell':
        ...
        sellcurr.loc[tradenum] = ([give_cur])
...
while done:
    ...
    if selected == 2:
        ...
        print(blotter) 
            ...
            if (sellcurr['Currency'].values in pls[['Ticker']].values) ==False and (givecurr['Currency'].values in plb[['Ticker']].values) ==True:
                pltot = plb[['Ticker','Current Market Price', 'Position', 'VWAP', 'URL', 'RPL','Total P/L']]
                prct = pltot.groupby(['Ticker']).agg({'Position': 'sum'})/blotter.agg({'Position': 'sum'})
            ...
            if (sellcurr['Currency'].values in plb[['Ticker']].values) == True and (givecurr['Currency'].values in pls[['Ticker']].values) ==True:
                pltot = df([[plb['Ticker'], 
                   quote[0], 
                ...
                pltot = plb[['Ticker','Current Market Price', 'Position', 'VWAP', 'URL', 'RPL','Total P/L']]
                prct = pltot.groupby(['Ticker']).agg({'Position': 'sum'})/pltot.agg({'Position': 'sum'})
                prctshare = pd.DataFrame(prct).reset_index()
                prctshare.columns = ['Ticker', '% of Total Shares']
                pltot = pd.merge(pltot, prctshare, on='Ticker')
                pltot.columns=['Ticker','Current Market Price','Position','VWAP','URL','RPL','Total P/L','% of Total Shares']
